...
window.Utils = (function(){
  ...
  let state = {
    /* Node; attributes -> [ID, x-coordinate, y-coordinate] */
    nodes: [],
    /* Edge; attributes -> [A-node ID, B-node ID, weight] */
    edges: [],
    /* Starting node*/
    sourceId: null,
    animating: false,
    cancelAnim: null,
    /* Node/Edge Identifiers <-depends- Hover Animation */
  ...
  function uniqueId(){
    const id = idFromIndex(state.nextId++);
    return id;
  }
  ...
  /* Self-explanatory */
  function clearHoverIfGone(){
    if (state.hover?.edge) {
      const e = state.hover.edge;
      const still = state.edges.some(x => (x.a===e.a && x.b===e.b) || (x.a===e.b && x.b===e.a));
      if (!still)
        state.hover.edge = null;
    }
    if (state.hover?.node) {
      const n = state.hover.node;
      const still = state.nodes.some(x => x.id === n.id);
      if (!still)
        state.hover.node = null;
    }
  }

  /* Geometry and UI rendering */
  function drawGraph(dist={}, prev={}, current=null, settled=new Set(), hiEdge=null){
    ...
    for (const e of edges){
      ...
      if (!state.animating && state.hover.edge === e) cls += ' hover';
      ...
      t.addEventListener('dblclick', (ev)=>{
        ...
        EdgeEditor.open(ev.clientX, ev.clientY, e, (newW)=>{
          e.w = newW;
          const res = runDijkstra(state.nodes, state.edges, state.sourceId);
          drawGraph(res.dist, res.prev, current, settled, hiEdge);
          renderTable(res.dist, res.prev, current, settled);
        });
      });
    ...
    /* CSS Rendering; Read Mozilla's documentation here -> https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Properties
       SVG Rendering; Read SVG documentation here -> http://www.w3.org/2000/svg
    Node Parameters (Radius... et cetaera);
    Label Parameters (Size... et cetaera); */
    for (const n of nodes){
      ...
      c.setAttribute('cx', n.x); c.setAttribute('cy', n.y); c.setAttribute('r', String(50));
      c.setAttribute('id', `node-${n.id}`);
      c.setAttribute('class', cls);
      state.svg.gNodes.appendChild(c);

      const lbl = document.createElementNS('http://www.w3.org/2000/svg','text');
      lbl.setAttribute('x', n.x); lbl.setAttribute('y', n.y - 18);
  ...
  function renderTable(dist, prev, current=null, settled=new Set()){
    ...
    state.tableEl.innerHTML = nodes.map(n=>{
      ...
      return `<tr class="${settled.has(n.id)?'settled':''} ${current===n.id?'current':''}">
        <td>${n.id}</td><td>${d}</td><td>${path}</td>
      </tr>`;
    }).join('');
  }
  ...
  function attachHoverHandlers(svgRoot){
    ...
    svgRoot.addEventListener('mouseleave', () => {
      ...
      state.hover.node = null; state.hover.edge = null;
      const r = runDijkstra(state.nodes, state.edges, state.sourceId);
      drawGraph(r.dist, r.prev);
    });
  }

  /* Small helper to pause hover during drags/connects */
  function setHoverLock(flag){ state.__lockHover = !!flag; }

  /* Geometry Hit */
  function ScreenToSVG(svgRoot, x, y){
    const pt = svgRoot.createSVGPoint(); pt.x=x; pt.y=y;
  ...
  function hitEdge(pt, nodes, edges, tol=6){
    for (const e of edges){
  ...
  /* Weight Value Edit Box  */
  const EdgeEditor = (function(){
    let el, input, dragging = false, startY = 0, base = 0;
    ...
    function ensure(){
      ...
      window.addEventListener('mousemove', ev=>{
        ...
        const r = Utils.runDijkstra(Utils.state.nodes, Utils.state.edges, Utils.state.sourceId);
        Utils.drawGraph(r.dist, r.prev);
        Utils.renderTable(r.dist, r.prev);
      });
